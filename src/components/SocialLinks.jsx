...
const SocialLinks = ({ className = "", iconSize = "w-6 h-6" }) => {
  const socialLinks = [
    {
      name: "LinkedIn",
    ...
    <div className={`flex space-x-4 sm:space-x-6 ${className}`}>
      {socialLinks.map((link) => (
        ...
        >
          ...
          <div className="transform group-hover:scale-110 transition-transform duration-200">{link.icon}</div>
        </a>
      ))}
    </div>
  );
